...
const MonochromeBox = ({ children }: { children: React.ReactNode }) => (
  <div className="modern:opacity-100 w-full h-full rounded-2xl bg-neutral-900 border border-neutral-800 flex items-center justify-center shadow-[inset_0_1px_1px_rgba(255,255,255,0.05)]">
    {children}
  </div>
);

const defaultItems: DockItemData[] = [
  { id: "home", label: "Home", icon: <MonochromeBox><PiHouseBold className="w-1/2 h-1/2 text-neutral-200" /></MonochromeBox> },
  { id: "layout", label: "Layout", icon: <MonochromeBox><PiSquaresFourBold className="w-1/2 h-1/2 text-neutral-200" /></MonochromeBox> },
  { id: "terminal", label: "Terminal", icon: <MonochromeBox><PiTerminalBold className="w-1/2 h-1/2 text-neutral-200" /></MonochromeBox> },
  { id: "sync", label: "Sync", icon: <MonochromeBox><PiArrowsClockwiseBold className="w-1/2 h-1/2 text-neutral-200" /></MonochromeBox> },
  { id: "x", label: "Twitter", icon: <MonochromeBox><PiXLogoBold className="w-1/2 h-1/2 text-neutral-200" /></MonochromeBox> },
  { id: "github", label: "GitHub", icon: <MonochromeBox><PiGithubLogoBold className="w-1/2 h-1/2 text-neutral-200" /></MonochromeBox> },
];

export interface MacDockMonochromeProps extends React.HTMLAttributes<HTMLDivElement> {
  items?: DockItemData[];
  className?: string;
}

export const MacDockMonochrome = ({
  items = defaultItems,
  className,
  ...props
}: MacDockMonochromeProps) => {
